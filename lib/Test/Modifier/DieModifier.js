...
describe('DieModifier', function () {
    ...
    it('should be serializable and deserializable', function () {
        var value = TestCore_1.TestCore.randomInt();
        var m = new DieModifier_1.DieModifier('Name', value);
        var serializedM = DieModifier_1.DieModifier.serialize(m);
        var ok = new DieModifier_1.DieModifier();
        var deserializedM = ok.deserialize(serializedM);
        chai_1.expect(deserializedM.getType()).to.be.equal('die');
        chai_1.expect(deserializedM.getPhase()).to.be.equal('before');
        chai_1.expect(deserializedM.getName()).to.be.equal('Name');
        chai_1.expect(deserializedM.getValue()).to.be.equal(value);
    });
});
