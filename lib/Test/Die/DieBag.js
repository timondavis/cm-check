...
describe('DieBag', function () {
    ...
    it('allows manipulation of the die bag using encoded die strings', function () {
        for (var pass = 0; pass < 1000; pass++) {
            bag = new DieBag_1.DieBag();
            var sidesA = TestCore_1.TestCore.randomInt(19) + 1;
            var sidesB = TestCore_1.TestCore.randomInt(19) + 1;
            var attempts = 0;
            while (sidesA === sidesB && attempts <= 1000) {
                sidesB = TestCore_1.TestCore.randomInt(19) + 1;
                attempts++;
            }
            if (attempts === 1000) {
                throw "Randomization is broken in DieBag Test";
            }
            var amountA = TestCore_1.TestCore.randomInt(19) + 1;
            var amountB = TestCore_1.TestCore.randomInt(19) + 1;
            var amountAdjustmentA = TestCore_1.TestCore.randomInt(amountA) - 1;
            var amountAdjustmentB = TestCore_1.TestCore.randomInt(amountB) - 1;
            ...
            chai_1.expect(bag.getDieWithSides(sidesA).length).to.be.equal(amountA - amountAdjustmentA);
            chai_1.expect(bag.getDieWithSides(sidesB).length).to.be.equal(amountB - amountAdjustmentB);
            var addAmountA = TestCore_1.TestCore.randomInt(10);
            var addAmountB = TestCore_1.TestCore.randomInt(10);
            bag.applyDieString(addAmountA + 'd' + sidesA);
            bag.applyDieString(addAmountB + 'd' + sidesB);
            chai_1.expect(bag.getDieWithSides(sidesA).length).to.be.equal(amountA + addAmountA - amountAdjustmentA);
            chai_1.expect(bag.getDieWithSides(sidesB).length).to.be.equal(amountB + addAmountB - amountAdjustmentB);
        }
    });
    it('can be serialized and deserialized (contents only, not state)', function () {
        var _loop_1 = function (i) {
            var freshBag = new DieBag_1.DieBag();
            var sidesCount = new Map();
            var addSize = TestCore_1.TestCore.randomInt(100);
            for (var i_1 = 0; i_1 < addSize; i_1++) {
                var sides = TestCore_1.TestCore.randomInt(20);
            ...
            var serializedBagString = DieBag_1.DieBag.serialize(freshBag);
            var rehydratedBag = DieBag_1.DieBag.deserialize(serializedBagString);
            Object.keys(rehydratedBag.dieMap).forEach(function (key) {
                chai_1.expect(rehydratedBag.dieMap[key].length).to.be.equal(sidesCount.get(Number.parseInt(key)));
                sidesCount.delete(Number.parseInt(key));
            });
            chai_1.expect(Object.keys(sidesCount).length).to.be.equal(0);
        };
