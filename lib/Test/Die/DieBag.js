...
describe('DieBag', function () {
    ...
    it('should add dice to the bag successfully', function () {
        ...
        bag.add(count, sides);
        chai_1.expect(bag.getSetOfSidedDie(sides)).has.lengthOf(count);
    });
    it('should facilitate adding dice en masse via import of a DieBag', function () {
        bag = new DieBag_1.DieBag();
        var mergeBag = new DieBag_1.DieBag();
        var bagAdds = {};
        for (var bagAddsLoop = 0; bagAddsLoop < TestCore_1.TestCore.randomInt(10); bagAddsLoop++) {
        ...
        Object.keys(bagAdds).forEach(function (dieSides) {
            chai_1.expect(bag.getSetOfSidedDie(dieSides)).to.have.lengthOf(bagAdds[dieSides]);
        });
    });
    ...
    it('should deliver all die filtered by # of sides correctly', function () {
        ...
        var sides2 = TestCore_1.TestCore.randomInt(100);
        var count3 = TestCore_1.TestCore.randomInt(100);
        var sides3 = TestCore_1.TestCore.randomInt(100);
        bag.add(count1, sides1);
        bag.add(count2, sides2);
        bag.add(count3, sides3);
        bag.roll();
        chai_1.expect(bag.getSetOfSidedDie(sides1).length).is.equal(count1);
        chai_1.expect(bag.getSetOfSidedDie(sides2).length).is.equal(count2);
        chai_1.expect(bag.getSetOfSidedDie(sides3).length).is.equal(count3);
    });
});
