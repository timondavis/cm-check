...
describe('DieBag', function () {
    ...
    function generateDieDefinitions() {
        ...
        var dieDefs = [];
        var sidesUsed = [];
        for (var defCounter = 0; defCounter < TestCore_1.TestCore.randomInt(20); defCounter++) {
            do {
                sides = TestCore_1.TestCore.randomInt();
            } while (sidesUsed.indexOf(sides) != -1);
            sidesUsed.push(sides);
            dieDefs.push([TestCore_1.TestCore.randomInt(), sides]);
        }
        return dieDefs;
    }
    function addDieToBagWithDefinitions(dieDefinitions, bag) {
