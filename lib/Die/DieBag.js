...
var DieBag = /** @class */ (function () {
    ...
    DieBag.prototype.add = function (count, sides, value) {
        ...
        if (count < 0) {
            throw ("Cannot add negative die amounts.  Use .remove() instead");
        }
     ...
     */
    DieBag.prototype.applyDieString = function (dieString) {
        var info = DieBag.decodeDieString(dieString);
    ...
    DieBag.prototype.remove = function (count, sides) {
        if (count < 0) {
            throw "Cannot remove negative amounts of die.  Use .add() instead";
        }
        if (!this.isDieIndexExists(sides)) {
            return this;
        }
     ...
     */
    DieBag.serialize = function (dieBag) {
        var _this = this;
        var arr = [];
        Object.keys(dieBag.dieMap).forEach(function (key) {
            if (dieBag.dieMap[key].length > 0) {
                arr.push(_this.encodeDieString(dieBag.dieMap[key].length, Number.parseInt(key)));
            }
        });
     ...
     */
    DieBag.deserialize = function (serialized) {
        var dieMap = JSON.parse(serialized);
        var d = new DieBag();
        var decoded = null;
    ...
    DieBag.encodeDieString = function (count, sides) {
        if (isNaN(count) || isNaN(sides)) {
